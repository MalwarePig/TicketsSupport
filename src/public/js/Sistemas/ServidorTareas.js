...
function ActualizarTareas() {
    ...
    $.post("/ActualizarTareasSistemas", // url
        {
            Registro
        }, // data to be submit
        function (objeto, estatus) { // success callback
           window.location.href = "/ListaServicioSistemas";
        }); 
}
...
function MostrarProyectos() {
    ...
    $.ajax({
        url: '/TareasAbiertasServidorSistemas/' + localStorage.getItem('Nomina'),
    ...
    alert.buttons = [
        {
            text: 'Cancelar',
            role: 'cancel',
            handler: () => { CerrarModalEstatusSistemas(); }
        },
        {
            text: 'OK',
            role: 'confirm',
            handler: () => {  CerrarTareasSistemas(); }
        }
    ];
